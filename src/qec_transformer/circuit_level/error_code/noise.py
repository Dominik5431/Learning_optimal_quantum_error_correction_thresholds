...
ANY_CLIFFORD_1_OPS = {"C_XYZ", "C_ZYX", "H", "H_YZ", "Y", "I", "SQRT_Y", "SQRT_Y_DAG"}
...
class NoiseModel:
    ...
    def noisy_op(self, op: stim.CircuitInstruction, p: float, ancilla: int) -> Tuple[
        stim.Circuit, stim.Circuit, stim.Circuit]:
        pre = stim.Circuit()
        mid = stim.Circuit()
        post = stim.Circuit()
        ...
        if p > 0:
            if op.name in ANY_CLIFFORD_1_OPS:
                ...
                if op.name in MEASURE_OPS:
                    ...
                    # pre.append_operation("DEPOLARIZE1", targets, p)
            elif op.name == "MPP":
