...
from ..dem.circuit_to_graph import DEM_Graph
...
class QecTransformer(Net):
    ...
    def __init__(self, name, distance,
        ...
        if readout == 'transformer-decoder':
            ...
            self.conv_to_data = nn.Conv2d(self.d_model, self.d_model, kernel_size=2, stride=1, padding=0, bias=False,
                                          groups=self.n_heads)
            self.to_log = nn.Conv2d(self.d_model, self.d_model, kernel_size=self.distance, stride=1, padding=0, bias=True)
    ...
    def input_repr(self, x):
        if self.measurement_input:
            ...
            if self.penc_type == 'learnable':
                ...
                event = scatter_to_2d(event, scatter_positions=self.scatter_indices, padding=torch.zeros(self.d_model), d=self.distance,
                                    device=self.device)
    ...
    def log_prob(self, x, include_mi=False):
        ...
        if self.pretraining:
            ...
            x_hat = self.encoder_forward(syndrome)
            if self.measurement_input:
                syndrome = syndrome[:, :, 0]
            ...
            if self.readout == 'transformer-decoder':
                ...
                sequence_length = 1
        if include_mi:
            return log_prob / sequence_length, torch.tensor(0)
            ...
            if self.readout == 'transformer-decoder':
                ...
                if self.measurement_input:
                    ...
                    start_token = torch.full((syndrome.size(0), 1, syndrome.size(2)), start_token_value, dtype=torch.long,
                                             device=self.device)
                    logical_in = torch.cat((start_token, syndrome[:, self.distance ** 2 - 1:, :]), dim=1).to(self.device)
